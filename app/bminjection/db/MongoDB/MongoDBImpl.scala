...
trait MongoDBImpl extends DBTrait {
    ...
    override def queryCount(condition: Imports.DBObject, db_name: String)
                           (implicit t: (Imports.DBObject) => Map[String, JsValue]): Option[Int] = {
        Some((from db() in db_name where condition).count)
    }

    override def deleteMultipleObject(condition: DBObject, db_name: String): Unit = {
        (from db() in db_name where condition select(x =>x)).toList match {
            case res => res.foreach(x => _data_connection.getCollection(db_name) -= x)
            case Nil => throw new Exception("no match data error")
        }
    }

    override def updateMultipleObject(condition: DBObject, doc: DBObject, db_name: String, isbatch: Boolean): Unit = {
        (from db() in db_name where condition select(x =>x)).toList match {
            case _ => {
                val value = new BasicDBObject("$set", doc)
                _data_connection.getCollection(db_name).update(condition, value, false ,isbatch)
            }
            case Nil => throw new Exception("no match data error")
        }
    }
