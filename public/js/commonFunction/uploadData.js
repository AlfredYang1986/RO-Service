...
$(document).ready(function(){
    $('#downloadBtn').click(function(){
        location = "/common/downloadFile/模板.xlsx";
    });
});
...
jQuery(function() {
    ...
    $('.uploder-container').each(function(index){
        ...
        var upload = $(this).find('.uploadBtn');                            //  : 上传按钮容器实例
        ...
        now_uploader = WebUploader.create({
            ...
            accept: {                               //  参数说明: {Arroy} [可选] [默认值：null] 指定接受哪些类型的文件.由于目前还有ext转mimeType表，所以这里需要分开指定.
                ...
                extensions: 'xlsx,xls',             //  参数说明: {String} 允许的文件后缀，不带点，多个用逗号分割.
                mimeTypes: '.xlsx,.xls'             //  参数说明: {String} 多个用逗号分割.
            },
        ...
        // XXX : 当文件被加入队列之前触发，此事件的handler返回值为false，则此文件不会被添加进入队列
        ...
        // XXX : 某个文件开始上传前触发，一个文件只会触发一次
        now_uploader.on('uploadStart', function(file){
            //uploadStart;
        });

        // XXX : 当某个文件的分块在发送前触发，主要用来询问是否要添加附带参数，大文件在开起分片上传的前提下此事件可能会触发多次
        now_uploader.on('uploadBeforeSend', function(object,data,headers){
            //uploadBeforeSend
            //console.info(object,data,headers)
        });

        // XXX : 当某个文件上传到服务端响应后，会派送此事件来询问服务端响应是否有效。如果此事件handler返回值为false, 则此文件将派送server类型的uploadError事件
        ...
        upload.on('click', function() {
            now_uploader.options.formData.user_name = "user_name";
            now_uploader.options.formData.product_name = "product_name";
            now_uploader.options.formData.create_date = "create_date";
