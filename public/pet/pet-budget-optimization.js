...
var bootstraptab = function() {
    ...
    $('#progressWizard').bootstrapWizard({
        ...
        onNext: function(tab, navigation, index) {
            try {
                if(index == 1){
                    verificationProductRatioSum()
                    UpdateProductOptimizatByProductBudget()
                    step(tab, navigation, index);
                    setTimeout(function () {
                        $(".next").attr("class", "next")
                    },200)
                }else{
                    verificatProductNowRatioSum()
                    location.href= "/astellas/resultsReport"
                }
            } catch (err) {
