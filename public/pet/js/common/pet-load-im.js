...
/**
 * 针对PET的Login获取WEB-IM的Token
 * token
 * @param u
 * @param p
 * @param o
 */
var login_im = function(u, p, o) {
    var options = {
        ...
        success: function(token) {
            $.tooltip("登录成功",2000,true,function(){
                $.closeDialog(function(){
                    $.session.set('token', o.token);
                    $.cookie('company', o.user.company);
                    $.cookie('department', o.user.department);
                    $.cookie('user_name', o.user.user_name);
                    $.cookie('level', o.user.is_admin);

                    $.cookie('webim_token', token.access_token);
                    $.cookie('webim_user', token.user.username);
                    window.location="/astellas/businessAnalysis"
                });
            });
        },
        error: function(m){
            $.tooltip('环信登录异常，请检查网络');
        ...
        onTextMessage: function ( message ) {
            ...
            if (ext !== null) {
                ...
                if(result === "progress") {
                    ...
                    else if(msg.startsWith("前面")){//需要排队
                    }
                    else if(msg === "优化完成" || msg === "调用出错"){
                        $('#queueLoding').modal('hide');
                        $.tooltip(msg, 2500, true);
                        querylist()
                    }
                    $('#chartMessage').html(msg);
                }
